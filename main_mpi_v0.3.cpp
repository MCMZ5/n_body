...
#define T          0.05  //Tempo totale di simulazione
#define debug      100
...
//inizializzo l'octree dividendo i corpi
  ...
  MPI_Bcast(&n, 8, MPI_INT, 0, MPI_COMM_WORLD); 

  int shift[8];
  shift[0]=0;
  for(int i=1;i<8;i++){
    shift[i]=shift[i-1]+N*i;
  } 
  MPI_Scatterv(octree,n,shift,Bodytype,bodies,n[myrank],Bodytype,0,MPI_COMM_WORLD);
  ...
  local_octree = Generate_Octree(n[myrank], ctr, l, bodies, myrank);  
...
//Esporto i risultati
  ...
  if (myrank==0) {
    ...
    if(Np>=2) save(output_file,bodies_end,N);
...
void Vectorial_Distance(Body b1, Body b2, double *d){
  ...
  d[0] = b[0]-a[0];
	d[1] = b[1]-a[1];
	d[2] = b[2]-a[2];
  if(isnan(d[0])==1 || isnan(d[1])==1 || isnan(d[2])==1){
        //cout << "Vectorial Nan rilevato" << "\n";
        //b1.Print();
        //b2.Print();
    }
